...
def bottleneck(inputs,
    ...
    with slim.arg_scope([prelu], decoder=decoder):
        ...
        #Everything is the same as a regular bottleneck except for a [5,5] kernel decomposed into two [5,1] then [1,5]
            ...
            # First projection with 1x1 kernel (dimensionality reduction)
            net = slim.conv2d(inputs, reduced_depth, [1, 1], scope=scope + 'fullconv2d_conv1')
            net = slim.batch_norm(net, is_training=is_training, scope=scope + 'fullconv2d_batch_norm1')
            net = prelu(net, scope=scope + '_prelu1')

            # Second conv block --- apply asymmetric conv here
            net = slim.conv2d(net, reduced_depth, [filter_size, filter_size], scope=scope + '_fullconv2d_conv2a')
            net = slim.batch_norm(net, is_training=is_training, scope=scope + 'fullconv2d_batch_norm2')
            net = prelu(net, scope=scope + '_prelu2')

            # Final projection with 1x1 kernel
            net = slim.conv2d(net, output_depth, [1, 1], scope=scope + 'fullconv2d_conv3')
            net = slim.batch_norm(net, is_training=is_training, scope=scope + 'fullconv2d_batch_norm3')
            net = prelu(net, scope=scope + '_prelu3')

            # Regularizer
            net = spatial_dropout(net, p=regularizer_prob, seed=seed, scope=scope + 'fullconv2d_spatial_dropout')
            net = prelu(net, scope=scope + 'fullconv2d_prelu4')

            # Add the main branch
            net = tf.add(net_main, net, name=scope + '_add_fullconv2d')
            net = prelu(net, scope=scope + 'fullconv2d_last_prelu')
        ...
        with slim.arg_scope([initial_block, bottleneck], is_training=is_training),\
            ...
            with slim.arg_scope([bottleneck], regularizer_prob=0.1):
                ...
                for i in range(2, max(stage_two_repeat, 1) + 2):#max(stage_two_repeat, 2) :
                    ...
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=3, scope='bottleneck'+str(i)+'_2')
                    net = bottleneck(net, output_depth=128, filter_size=5, fullconv2d=False,asymmetric=True, scope='bottleneck'+str(i)+'_3')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=5, scope='bottleneck'+str(i)+'_4')
                    net = bottleneck(net, output_depth=128, filter_size=3, scope='bottleneck'+str(i)+'_5')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=9, scope='bottleneck'+str(i)+'_6')
                    net = bottleneck(net, output_depth=128, filter_size=5, fullconv2d=False,asymmetric=True, scope='bottleneck'+str(i)+'_7')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=11, scope='bottleneck'+str(i)+'_8')

                    net = bottleneck(net, output_depth=128, filter_size=3, scope='bottleneck' + str(i) + '_1_1')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=2,scope='bottleneck' + str(i) + '_2_1')
                    net = bottleneck(net, output_depth=128, filter_size=5, fullconv2d=False, asymmetric=True,scope='bottleneck' + str(i) + '_3_1')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=5,scope='bottleneck' + str(i) + '_4_1')
                    net = bottleneck(net, output_depth=128, filter_size=3, scope='bottleneck' + str(i) + '_5_1')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=9,scope='bottleneck' + str(i) + '_6_1')
                    net = bottleneck(net, output_depth=128, filter_size=5, fullconv2d=False, asymmetric=True,scope='bottleneck' + str(i) + '_7_1')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=11,scope='bottleneck' + str(i) + '_8_1')
