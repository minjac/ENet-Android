...
def unpool(updates, mask, k_size=[1, 2, 2, 1], output_shape=None, scope='', ksize=None):#反池化，updates为待处理矩阵，mask为索引
    '''
    ...
    with tf.variable_scope(scope):
        ...
        one_like_mask = tf.ones_like(mask, dtype=tf.int32)#全部的值都设为1，保持 type 和shape 不变
        batch_shape = tf.concat([[input_shape[0]], [1], [1], [1]], 0)#拼接张量，0为axis,[25,1,1,1]
        batch_range = tf.reshape(tf.range(output_shape[0], dtype=tf.int32), shape=batch_shape)#output_shape[0]=25
        b = one_like_mask * batch_range #
        #y = mask // (output_shape[2] * output_shape[3])
        y = mask // (output_shape[2] * output_shape[3]) % output_shape[1] #y轴 height
        x = (mask // output_shape[3]) % output_shape[2] #x轴 width mask %(output_shape[2] * output_shape[3]) // output_shape[3]
        feature_range = tf.range(output_shape[3], dtype=tf.int32) #channel（0，1，2）
        f = one_like_mask * feature_range

        # transpose indices & reshape update values to one dimension
        updates_size = tf.size(updates) #返回tensor大小，数据为int，非数组
        indices = tf.transpose(tf.reshape(tf.stack([b, y, x, f]), [4, updates_size]))#stack增加阶数
...
def bottleneck(inputs,
    ...
    with slim.arg_scope([prelu], decoder=decoder):
        ...
        if downsampling:
            ...
            #net_main, pooling_indices最大池化数值及其索引
...
def ENet(inputs,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        with slim.arg_scope([initial_block, bottleneck], is_training=is_training),\
            ...
            '''conv_1x1 = layers_lib.conv2d(net, 64, [1, 1], stride=1, scope="conv_1x1")
            conv_3x3_1 = resnet_utils.conv2d_same(net, 64, 3, stride=1, rate=6, scope='conv_3x3_1')
            conv_3x3_2 = resnet_utils.conv2d_same(net, 64, 3, stride=1, rate=12, scope='conv_3x3_2')
            conv_3x3_3 = resnet_utils.conv2d_same(net, 64, 3, stride=1, rate=18, scope='conv_3x3_3')
            net = tf.concat([conv_1x1, conv_3x3_1, conv_3x3_2, conv_3x3_3, ], axis=3, name='concat')
            net = layers_lib.conv2d(net, 64, [1, 1], stride=1, scope='conv_1x1_concat')'''
            '''net_ap0 = slim.conv2d(net, 16, [1, 1], scope=scope + 'a_conv1')
            net_ap0 = slim.batch_norm(net_ap0, is_training=is_training, scope=scope + 'a_batch_norm1')
            net_ap0 = prelu(net_ap0, scope=scope  + 'a_prelu1')
            # net_ap0 = slim.conv2d(net_ap0, 32, [1, 1], scope=scope + 'ap0_conv2d')
            net_ap1 = bottleneck(net_ap0, output_depth=16, filter_size=3, dilated=True, dilation_rate=1,
                                 scope='bottleneck' + 'ap_1')
            net_ap2 = bottleneck(net_ap0, output_depth=16, filter_size=3, dilated=True, dilation_rate=6,
                                 scope='bottleneck'  + 'ap_2')
            net_ap3 = bottleneck(net_ap0, output_depth=16, filter_size=3, dilated=True, dilation_rate=12,
                                 scope='bottleneck'+ 'ap_3')
            net_ap4 = bottleneck(net_ap0, output_depth=16, filter_size=3, dilated=True, dilation_rate=18,
                                 scope='bottleneck' + 'ap_4')
            net_ap = tf.concat([net_ap1, net_ap2, net_ap3, net_ap4], axis=3, name='bottleneck' + str(i) + '_concat')

            net = bottleneck(net_ap, output_depth=64, filter_size=3, regularizer_prob=0.01, scope='bottleneck1_1')'''
            #net = bottleneck(net, output_depth=64, filter_size=2, regularizer_prob=0.01, scope='bottleneck1_2')
            ...
            with slim.arg_scope([bottleneck], regularizer_prob=0.1):
                ...
                #net = slim.conv2d(net, 128, [3, 3], stride=2, scope=scope + '_conv_down')
                #net = tf.nn.conv2d(net, filter=[2,2,64,128],strides=[1, 2, 2, 1],padding='VALID',name=scope + '_transposed_conv2_down')
                #Repeat the stage two at least twice to get stage 2 and 3:
                for i in range(2, max(stage_two_repeat, 1) + 2):#max(stage_two_repeat, 2) :
                    '''
                    net = bottleneck(net_ap, output_depth=128, filter_size=3, scope='bottleneck'+str(i)+'_1')
                    ...
                    net = bottleneck(net, output_depth=128, filter_size=3, scope='bottleneck'+str(i)+'_5')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=8, scope='bottleneck'+str(i)+'_6')
                    net = bottleneck(net, output_depth=128, filter_size=10, asymmetric=True, scope='bottleneck'+str(i)+'_7')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=16, scope='bottleneck'+str(i)+'_8')

                    net = bottleneck(net, output_depth=128, filter_size=3, scope='bottleneck'+str(i)+'_a1')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=2, scope='bottleneck'+str(i)+'_a2')
                    net = bottleneck(net, output_depth=128, filter_size=5, asymmetric=True, scope='bottleneck'+str(i)+'_a3')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=4, scope='bottleneck'+str(i)+'_a4')'''

                    '''net = bottleneck(net, output_depth=128, filter_size=3, scope='bottleneck' + str(i) + '_1')
                    net = bottleneck(net, output_depth=128, filter_size=5, asymmetric=True,
                                     scope='bottleneck' + str(i) + '_2')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=3,
                                     scope='bottleneck' + str(i) + '_3')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=5,
                                     scope='bottleneck' + str(i) + '_4')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=7,
                                     scope='bottleneck' + str(i) + '_5')
                    net = bottleneck(net, output_depth=128, filter_size=3, scope='bottleneck' + str(i) + '_6')

                    net = bottleneck(net, output_depth=128, filter_size=5, asymmetric=True,
                                     scope='bottleneck' + str(i) + '_7')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=9,
                                     scope='bottleneck' + str(i) + '_8')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=11,
                                     scope='bottleneck' + str(i) + '_9')
                    net = bottleneck(net, output_depth=128, filter_size=3, dilated=True, dilation_rate=13,
                                     scope='bottleneck' + str(i) + '_10')'''


                #asspmogai-----------------------------------------------------------------
                net_1 = bottleneck(net, output_depth=128, filter_size=3, scope='bottleneck' + str(i) + '1_1')
                net_1 = bottleneck(net_1, output_depth=128, filter_size=3, dilated=True, dilation_rate=2, scope='bottleneck' + str(i) + '1_2')
                net_1 = bottleneck(net_1, output_depth=128, filter_size=5, asymmetric=True,scope='bottleneck' + str(i) + '1_3')
                net_1 = bottleneck(net_1, output_depth=128, filter_size=3, dilated=True, dilation_rate=4,scope='bottleneck' + str(i) + '1_4')

                net_2, pooling_indices1_1, inputs_shape1_1 = bottleneck(net_1, output_depth=256, filter_size=3, regularizer_prob=0.01, downsampling=True, scope='bottleneck1_1_0')

                net_2 = bottleneck(net_2, output_depth=256, filter_size=3, scope='bottleneck' + str(i) + '2_1')
                net_2 = bottleneck(net_2, output_depth=256, filter_size=3, dilated=True, dilation_rate=2,scope='bottleneck' + str(i) + '2_2')
                net_2 = bottleneck(net_2, output_depth=256, filter_size=5, asymmetric=True,scope='bottleneck' + str(i) + '2_3')
                net_2 = bottleneck(net_2, output_depth=256, filter_size=3, dilated=True, dilation_rate=4,scope='bottleneck' + str(i) + '2_4')

                net_3, pooling_indices2_1, inputs_shape2_1 = bottleneck(net_2, output_depth=512, filter_size=3, regularizer_prob=0.01, downsampling=True, scope='bottleneck2_1_0')

                net_3 = bottleneck(net_3, output_depth=512, filter_size=3, scope='bottleneck' + str(i) + '3_1')
                net_3 = bottleneck(net_3, output_depth=512, filter_size=3, dilated=True, dilation_rate=2,scope='bottleneck' + str(i) + '3_2')
                net_3 = bottleneck(net_3, output_depth=512, filter_size=5, asymmetric=True, scope='bottleneck' + str(i) + '3_3')
                net_3 = bottleneck(net_3, output_depth=512, filter_size=3, dilated=True, dilation_rate=4,scope='bottleneck' + str(i) + '3_4')

                net_4, pooling_indices3_1, inputs_shape3_1 = bottleneck(net_3, output_depth=1024, filter_size=3, regularizer_prob=0.01, downsampling=True, scope='bottleneck3_1_0')


                net_4 = bottleneck(net_4, output_depth=1024, filter_size=3, scope='bottleneck' + str(i) + '4_1')
                net_4 = bottleneck(net_4, output_depth=1024, filter_size=3, dilated=True, dilation_rate=2,scope='bottleneck' + str(i) + '4_2')
                net_4 = bottleneck(net_4, output_depth=1024, filter_size=5, asymmetric=True, scope='bottleneck' + str(i) + '4_3')
                net_4 = bottleneck(net_4, output_depth=1024, filter_size=3, dilated=True, dilation_rate=4,scope='bottleneck' + str(i) + '4_4')
                #net_4, pooling_indices4_1, inputs_shape4_1 = bottleneck(net, output_depth=64, filter_size=3, regularizer_prob=0.01, downsampling=True, scope='bottleneck4_1_0')
...
#asspmogai------------------------------------------------------------------------
                ...
                net_dc4 = bottleneck(net_4, output_depth=512, filter_size=3, upsampling=True,pooling_indices=pooling_indices3_1, output_shape=inputs_shape3_1, scope=bottleneck_scope_name+'4_0')

                net_dc3 = tf.add(net_dc4, net_3, name=scope+'dc4_add')
                net_dc3 = prelu(net_dc3, scope=scope+'dc4_last_prelu')
                net_dc3 = bottleneck(net_dc3, output_depth=256, filter_size=3, upsampling=True,pooling_indices=pooling_indices2_1, output_shape=inputs_shape2_1, scope=bottleneck_scope_name+'3_0')

                net_dc2 = tf.add(net_dc3, net_2, name=scope+'dc3_add')
                net_dc2 = prelu(net_dc2, scope=scope+'dc3_last_prelu')
                net_dc2 = bottleneck(net_dc2, output_depth=128, filter_size=3, upsampling=True,pooling_indices=pooling_indices1_1, output_shape=inputs_shape1_1, scope=bottleneck_scope_name+'2_0')

                net_dc1 = tf.add(net_dc2, net_1, name=scope+'dc2_add')
                net_dc1 = prelu(net_dc1, scope=scope+'dc2_last_prelu')
                net_dc1 = bottleneck(net_dc1, output_depth=64, filter_size=3, upsampling=True,pooling_indices=pooling_indices_2, output_shape=inputs_shape_2, scope=bottleneck_scope_name+'1_0')
...
#-------------------------------------------------------------------------------
                ...
                #net = bottleneck(net, output_depth=64, filter_size=3, upsampling=True,pooling_indices=pooling_indices_2, output_shape=inputs_shape_2, scope=bottleneck_scope_name+'_0')
                #net = slim.conv2d(net, 64, [3, 3], scope=scope + '_conv_up')
                ...
                net_dc1 = bottleneck(net_dc1, output_depth=64, filter_size=3, scope=bottleneck_scope_name + '_1')
                net_dc1 = bottleneck(net_dc1, output_depth=64, filter_size=3, scope=bottleneck_scope_name + '_2')

                #===================STAGE FIVE========================
                bottleneck_scope_name = "bottleneck" + str(i + 2)

                net = bottleneck(net_dc1, output_depth=16, filter_size=3, upsampling=True,
