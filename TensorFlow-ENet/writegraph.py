...
image = './dataset/test/0001TP_008550.png'
#images_list = sorted([os.path.join(image_dir, file) for file in os.listdir(image_dir) if file.endswith('.png')])
checkpoint_dir="D:\\TensorFlow-ENet-master\\checkpoint"
pb_path = "D:\\TensorFlow-ENet-master\\checkpoint\\enet_test.pb"
...
with tf.Graph().as_default() as graph:
    ...
    images = tf.train.batch([image], batch_size=25, allow_smaller_final_batch=True)
    ...
    with tf.device('/gpu:0'):
        with tf.Graph().as_default():
            ...
            with tf.Session(config=config) as sess:
                ...
                input_image_tensor = sess.graph.get_tensor_by_name("batch_1/fifo_queue:0")#
                #softmax = sess.graph.get_tensor_by_name("softmax/softmax:0")
                probabilities = sess.graph.get_tensor_by_name("Images/Validation_segmentation_output:0")
                probabilities = sess.run(probabilities, feed_dict={
                    input_image_tensor: img})  # out1, out2 = sess.run([softmax, conv62], feed_dict = {input_image_tensor:img_x})
